...
void DrawClock(int screenWidth, int screenHeight) {
    ...
    float hourAngle = (ltm->tm_hour % 12 + ltm->tm_min / 60.0) * 30.0 * DEG2RAD - PI / 2;
    float minuteAngle = (ltm->tm_min + ltm->tm_sec / 60.0) * 6.0 * DEG2RAD - PI / 2;
    float secondAngle = ltm->tm_sec * 6.0 * DEG2RAD - PI / 2;
    ...
    for (int i = 0; i < 12; ++i) {
        float angle = i * 30.0 * DEG2RAD - PI / 2;
